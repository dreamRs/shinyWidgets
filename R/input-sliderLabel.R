#' @title Slider Label Input Widget
#'
#' @description Constructs a slider widget with numeric values and character labels
#'
#' @param inputId The \code{input} slot that will be used to access the value.
#' @param label Display label for the control, or \code{NULL} for no label.
#' @param choices Named numeric vector
#' @param selected The initially selected numeric values, if length > 1, create a range slider.
...
#'
#' @seealso \link{updateSliderLabelInput} to update value server-side.
#'
#' @return The value retrieved server-side is a numeric value.
#' @export
#'
...
#'
#' }
sliderLabelInput <- function (inputId, label, choices, selected = NULL,
                             ...
                             width = NULL, pre = NULL, post = NULL, dragRange = TRUE)
{
  ...
  sliderProps <- dropNulls(list(
    ...
    #`data-swvalues` = enc2native(jsonlite::toJSON(x = names(choices))),
    `data-values` = paste0(names(choices), collapse = ','),
    `data-keyboard` = TRUE,
  ...
  dep <- htmltools::findDependencies(shiny::sliderInput(inputId = "id", label = "label", min = min(choices), max = max(choices), value = selected))
...
#' @title Change the value of a slider label input on the client
#'
#' @description
...
#'
#' }
updateSliderLabelInput <- function (session, inputId, label = NULL, selected = NULL, choices = NULL, from_fixed = NULL, to_fixed = NULL) {
