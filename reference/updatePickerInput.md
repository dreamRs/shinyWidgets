...
- selected:
  ...
  The new selected value (or multiple values if `multiple = TRUE`). To
  reset selected value, in case of multiple picker, use `character(0)`.
...
- clearOptions:
  ...
  Clear previous options, otherwise the ones set previously are still
  active.
